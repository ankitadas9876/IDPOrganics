...
'''Script to calculate remaining Organic at Juventae Chasma
Latitude: -3.6
'''
...
temp = pd.DataFrame(np.loadtxt('daytemp_juventae.txt'))
dfd = pd.DataFrame(np.loadtxt('juventae.txt'))
dep = np.array(dfd[1])
ls = np.array(dfd[0])
...
for i in range(len(energy_c)):
    if lat[i] == -10:
        ...
        e_new_c[j] = np.interp(lat_juv,x,y)
...
for i in range(len(energy_b)):
    if lat[i] == -10:
        ...
        e_new_b[j] = np.interp(lat_juv,x,y)
...
for i in range(len(energy_a)):
    if lat[i] == -10:
        ...
        e_new_a[j] = np.interp(lat_juv,x,y)
        e_totala = e_totala + e_new_a[j]
        j = j+1
Ls = np.array(list(range(0,360,10))) #solar longitude (Ls)
...
FUV_avg = 59354500/36 #averaged UV ABC flux for 36 solar longitude points
seconds_Myr = 88775
...
plt.title('Juventae Chasma',size=20)
plt.yticks([0.01,0.02,0.03,0.04,0.06], ['0.01','0.02','0.03','0.04','0.06'])
plt.tight_layout
plt.savefig('JuventaeOrganics_Das.png')
