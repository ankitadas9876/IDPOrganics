...
'''Script to calculate remaining Organic at Arabia Terra
Latitude: 21
'''
...
temp = pd.DataFrame(np.loadtxt('daytemp_arabiaterra.txt'))
dfd = pd.DataFrame(np.loadtxt('arabiaterra.txt'))
dep = np.array(dfd[1])
ls = np.array(dfd[0])
...
for i in range(len(energy_c)):
    if lat[i] == 20:
        ...
        e_new_c[j] = np.interp(lat_arb,x,y)
...
for i in range(len(energy_b)):
    if lat[i] == 20:
        ...
        e_new_b[j] = np.interp(lat_arb,x,y)
...
for i in range(len(energy_a)):
    if lat[i] == 20:
        ...
        e_new_a[j] = np.interp(lat_arb,x,y)
...
plt.title('Arabia Terra',size=20)
plt.yticks([0.01,0.02,0.03,0.04, 0.046], ['0.01','0.02','0.03','0.04','0.046'])
plt.tight_layout
plt.savefig('ArabiaOrganics_Das.png')
